...
| pytest (Windows) | 1949 passed, 2 skipped, 1 documented environment exception |
...
Only one of the two currently fails on this host: the content-addressed
artifact happens to hold LF bytes on disk, so its digest test passes here. That
is incidental, not a fix — a fresh clone with `core.autocrlf=true` and no
`.gitattributes` would smudge it again.
