...
class MusicalEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.duration_ticks, bool) or not isinstance(self.duration_ticks, int):
            raise SpatialMappingError("duration_ticks must be an integer")
        if self.duration_ticks <= 0:
            raise SpatialMappingError("duration_ticks must be positive")
        if (
            isinstance(self.velocity, bool)
            or not isinstance(self.velocity, int)
            or not 0 <= self.velocity <= 127
        ):
            raise SpatialMappingError("velocity must be an integer between 0 and 127")
